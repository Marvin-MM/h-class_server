...
export function createApp(container: Container) {
  ...
  // ─── Rate Limiters ─────────────────────────────────────────────
  const authMiddleware = createAuthMiddleware(container.config, container.redis);
  const { apiLimiter, authLimiter, uploadLimiter } = createRateLimiters(container.redis);
  ...
  app.use(`${v1}/auth`, createAuthRouter(container.authController, authMiddleware, authLimiter));
  app.use(`${v1}/users`, createUsersRouter(container.usersController, authMiddleware, uploadLimiter));
  app.use(`${v1}/media`, createMediaRouter(container.mediaController, authMiddleware, uploadLimiter));
  app.use(`${v1}/courses`, createCoursesRouter(container.coursesController, authMiddleware));
  app.use(`${v1}/payments`, createPaymentsRouter(container.paymentsController, authMiddleware));
  app.use(`${v1}/sessions`, createSessionsRouter(container.sessionsController, authMiddleware));
  app.use(`${v1}/notes`, createNotesRouter(container.notesController, authMiddleware));
  app.use(`${v1}/assessments`, createAssessmentsRouter(container.assessmentsController, authMiddleware, uploadLimiter));
  app.use(`${v1}/certificates`, createCertificatesRouter(container.certificatesController, authMiddleware));
  app.use(`${v1}/chat`, createChatRouter(container.chatController, authMiddleware));
  app.use(`${v1}/notifications`, createNotificationsRouter(container.notificationsController, authMiddleware));
  app.use(`${v1}/domains`, createDomainsRouter(container.domainsController, authMiddleware));
  app.use(`${v1}/calendar`, createCalendarRouter(container.calendarController, authMiddleware));
