...
import { eventBus, AppEvents, type CertificateEligiblePayload } from '../../shared/utils/event-bus.js';
...
export class CertificatesService {
  ...
  async getCertificate(certificateId: string, userId: string): Promise<CertificateResponse> {
    ...
    if (cert.studentId !== userId) throw new AuthorizationError('You can only view your own certificates');
    return this.toResponse(cert);
  }

  /** Gets all certificates for a student. */
  async getStudentCertificates(studentId: string): Promise<CertificateResponse[]> {
  ...
  async handleEligibility(payload: CertificateEligiblePayload): Promise<void> {
    ...
    if (existing) {
      logger.info('Certificate already exists for student/course', { studentId, courseId });
      return;
    }

    // Create PENDING certificate record
    const cert = await this.certificatesRepository.create({ studentId, courseId });
    ...
    logger.info('Certificate record created and compilation queued', { certificateId: cert.id });
  }

  private registerEventListeners(): void {
    eventBus.on(AppEvents.CERTIFICATE_ELIGIBLE, (payload) => {
      this.handleEligibility(payload).catch((error) => {
        logger.error('Failed to handle certificate eligibility', { error, payload });
      });
    });
  }
