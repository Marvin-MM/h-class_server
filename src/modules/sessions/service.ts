...
export class SessionsService {
  ...
  async createSession(tutorId: string, dto: CreateSessionDto): Promise<SessionResponse> {
    const course = await this.coursesRepository.findById(dto.courseId);
    if (!course) throw new NotFoundError('Course', dto.courseId);
    if (course.tutorId !== tutorId) throw new AuthorizationError('You can only create sessions for your own courses');

    // Check that the course has at least one enrolled student
    const enrollmentCount = await this.coursesRepository.countEnrollments(dto.courseId);
    if (enrollmentCount === 0) {
      throw new ValidationError('Cannot create a session for a course with no enrolled students');
    }
    ...
    logger.info('Session created', { sessionId: session.id, courseId: dto.courseId });
  ...
  async startSession(sessionId: string, tutorId: string): Promise<SessionResponse> {
    const session = await this.sessionsRepository.findById(sessionId);
    if (!session) throw new NotFoundError('Session', sessionId);

    const course = await this.coursesRepository.findById(session.courseId);
    if (!course || course.tutorId !== tutorId) throw new AuthorizationError('You can only start sessions for your own courses');
    if (session.status !== 'SCHEDULED') throw new ConflictError('Only SCHEDULED sessions can be started');

    const updated = await this.sessionsRepository.updateStatus(sessionId, 'LIVE');
    ...
    eventBus.emit(AppEvents.SESSION_STARTED, { sessionId, courseId: session.courseId });
    logger.info('Session started', { sessionId });
    return this.toResponse(updated);
  }

  /** Ends a session (LIVE → ENDED). */
  async endSession(sessionId: string, tutorId: string): Promise<SessionResponse> {
    const session = await this.sessionsRepository.findById(sessionId);
    if (!session) throw new NotFoundError('Session', sessionId);

    const course = await this.coursesRepository.findById(session.courseId);
    if (!course || course.tutorId !== tutorId) throw new AuthorizationError('You can only end sessions for your own courses');
    if (session.status !== 'LIVE') throw new ConflictError('Only LIVE sessions can be ended');

    const updated = await this.sessionsRepository.updateStatus(sessionId, 'ENDED');
    eventBus.emit(AppEvents.SESSION_ENDED, { sessionId, courseId: session.courseId });
    logger.info('Session ended', { sessionId });
    return this.toResponse(updated);
  }

  /** Generates a GetStream Video join token for a user. */
  async joinSession(sessionId: string, userId: string, userRole: string): Promise<JoinSessionResult> {
    const session = await this.sessionsRepository.findById(sessionId);
    if (!session) throw new NotFoundError('Session', sessionId);
    if (!session.getStreamCallId) throw new ConflictError('No video call configured for this session');
    ...
    if (userRole === 'TUTOR' && course.tutorId !== userId) {
      throw new AuthorizationError('You can only join sessions for your own courses');
    }
    if (userRole === 'STUDENT') {
      const isEnrolled = await this.coursesRepository.isEnrolled(userId, session.courseId);
      if (!isEnrolled) throw new AuthorizationError('You must be enrolled in the course to join this session');
    }

    // Generate GetStream Video token
    const exp = Math.floor(Date.now() / 1000) + 3600; // 1 hour
    const iat = Math.floor(Date.now() / 1000) - 10;
    const token = this.streamVideoClient.generateUserToken({ user_id: userId, exp, iat });
