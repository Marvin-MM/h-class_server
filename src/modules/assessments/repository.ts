import type { PrismaClient, Assessment, Submission, AssessmentType } from '@prisma/client';

export class AssessmentsRepository {
  constructor(private readonly prisma: PrismaClient) {}

  async create(data: { courseId: string; title: string; type: AssessmentType }): Promise<Assessment> {
  ...
  async findByCourseId(courseId: string): Promise<Assessment[]> {
    return this.prisma.assessment.findMany({ where: { courseId }, orderBy: { createdAt: 'asc' } });
  }
  ...
  async createSubmission(data: { assessmentId: string; studentId: string; s3Key: string }): Promise<Submission> {
  ...
  async findSubmissionByAssessmentAndStudent(assessmentId: string, studentId: string): Promise<Submission | null> {
    return this.prisma.submission.findUnique({
      where: { assessmentId_studentId: { assessmentId, studentId } },
    });
  }

  async gradeSubmission(id: string, score: number, feedback?: string): Promise<Submission> {
  ...
  async getStudentCourseSubmissions(courseId: string, studentId: string): Promise<{
  ...
  async findSubmissionsByAssessmentId(assessmentId: string): Promise<Submission[]> {
