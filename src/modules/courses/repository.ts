...
export class CoursesRepository {
  ...
  async findByIdWithTutor(id: string): Promise<(Course & { tutor: { id: string; firstName: string; lastName: string } }) | null> {
    return this.prisma.course.findFirst({
      where: { id, deletedAt: null },
      include: {
        tutor: {
  ...
  async findMany(options: {
    ...
    tutorId?: string;
    search?: string;
  }): Promise<PaginatedResult<Course & { tutor: { id: string; firstName: string; lastName: string } }>> {
  ...
  /** Updates a course. */
  async update(id: string, data: Partial<Pick<Course, 'title' | 'description' | 'price' | 'passMark' | 'status'>>): Promise<Course> {
  ...
  async createEnrollment(data: { userId: string; courseId: string }): Promise<Enrollment> {
  ...
  async findEnrollments(courseId: string, pagination: PaginationOptions): Promise<PaginatedResult<Enrollment & { user: { id: string; firstName: string; lastName: string; email: string } }>> {
  ...
  async findStudentEnrollments(userId: string, pagination: PaginationOptions): Promise<PaginatedResult<Enrollment & { course: { title: string; status: string } }>> {
    const where = { userId };
    const [data, total] = await this.prisma.$transaction([
      this.prisma.enrollment.findMany({
        where,
  ...
  async findByIdForUpdate(id: string, tx: Prisma.TransactionClient): Promise<Course | null> {
