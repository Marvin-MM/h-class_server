...
export class DomainsController {
  ...
  request = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const result = await this.domainsService.requestDomain(req.user!.userId, req.body as RequestDomainDto);
      sendSuccess(res, result, 201);
    } catch (error) { next(error); }
  };

  getMyDomain = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
