...
export class DomainsRepository {
  ...
  async isReserved(subdomain: string): Promise<boolean> {
    const reserved = await this.prisma.reservedSubdomain.findUnique({ where: { subdomain } });
  ...
  async updateStatus(id: string, status: DomainStatus, cloudflareDnsRecordId?: string): Promise<Domain> {
    return this.prisma.domain.update({
      where: { id },
      data: { status, ...(cloudflareDnsRecordId ? { cloudflareDnsRecordId } : {}) },
    });
  }
