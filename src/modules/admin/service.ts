...
export class AdminService {
  ...
  async getApplications(status?: string, page = 1, pageSize = 20) {
    const result = await this.adminRepository.findApplications(
      status,
      page,
      pageSize,
    );
  ...
  async approveApplication(applicationId: string): Promise<void> {
    ...
    await this.emailQueue.add("tutor-approved", {
      to: app.user.email,
    ...
    eventBus.emit(AppEvents.TUTOR_APPLICATION_APPROVED, {
      applicationId,
      userId: app.userId,
      email: app.user.email,
      firstName: app.user.firstName,
    });
    logger.info("Tutor application approved", {
      applicationId,
      userId: app.userId,
    });
  }
  ...
  async denyApplication(
    ...
    dto: ApplicationActionDto,
  ): Promise<void> {
    const app = await this.adminRepository.findApplicationById(applicationId);
    if (!app) throw new NotFoundError("Application", applicationId);
    if (app.status !== "PENDING")
      throw new ConflictError("Only PENDING applications can be denied");
    ...
    await this.notificationsService.notify(
      ...
      `Your application was denied: ${dto.reason}`,
    );
    await this.emailQueue.add("tutor-denied", {
      to: app.user.email,
      subject: "Tutor Application Update",
      template: "tutor-denied",
      data: { firstName: app.user.firstName, reason: dto.reason },
    });

    eventBus.emit(AppEvents.TUTOR_APPLICATION_DENIED, {
      applicationId,
      userId: app.userId,
      email: app.user.email,
      firstName: app.user.firstName,
    ...
    logger.info("Tutor application denied", {
      applicationId,
      userId: app.userId,
    });
  }
  ...
  async createConfig(dto: CreateConfigDto): Promise<AppConfigResponse> {
    const existing = await this.adminRepository.findConfigByKey(dto.key);
    if (existing) {
      throw new ConflictError(
        `Config key "${dto.key}" already exists. Use PATCH to update it.`,
      );
    }

    const config = await this.adminRepository.createConfig(dto.key, dto.value);
    logger.info("Platform config created", { key: dto.key });
    return {
      key: config.key,
      value: config.value,
  ...
  async updateConfig(dto: UpdateConfigDto): Promise<AppConfigResponse> {
    const existing = await this.adminRepository.findConfigByKey(dto.key);
    if (!existing) {
      throw new NotFoundError("Config", dto.key);
    }
  ...
  async deleteConfig(key: string): Promise<void> {
    const existing = await this.adminRepository.findConfigByKey(key);
    if (!existing) {
      throw new NotFoundError("Config", key);
    }

    await this.adminRepository.deleteConfig(key);
