...
export function createQueues(options: QueueFactoryOptions) {
  ...
  const connection = options.connection as unknown as import('bullmq').QueueOptions['connection'];

  const emailQueue = new Queue(EMAIL_QUEUE, { connection });
  const pushNotificationQueue = new Queue(PUSH_NOTIFICATION_QUEUE, { connection });
  const domainProvisioningQueue = new Queue(DOMAIN_PROVISIONING_QUEUE, { connection });
  const certificateCompilationQueue = new Queue(CERTIFICATE_COMPILATION_QUEUE, { connection });
  const auditArchiveQueue = new Queue(AUDIT_ARCHIVE_QUEUE, { connection });
  const sessionReminderQueue = new Queue(SESSION_REMINDER_QUEUE, { connection });
  const pushTokenCleanupQueue = new Queue(PUSH_TOKEN_CLEANUP_QUEUE, { connection });
